...
<body>
  <main>
    ...
    <p>
      <code>database.jpg</code> is a real photo. You can open it like any
      picture. It is also a real database. After the picture data ends, the
      file holds real microscope images of human cells, real measurements,
      and embeddings from a pretrained model, each one a list of numbers
      that describes what a cell looks like.
    </p>
    <p>
      We use DuckDB, a database engine that can run inside your browser, so
      you can ask the data real questions with SQL, the language you use to
      query a database. Then you can watch how we are able to use MorphEm,
      a pretrained model, to group similar-looking cells together, just by
      hovering or clicking cells in the plots below.
    </p>
    <img class="cover" src="database.jpg" alt="Database JPEG cover image">
    <div class="panel">
      <h2>Run your own SQL</h2>
      <p>
        Start with a question. This box runs real SQL against the
        database, using DuckDB-Wasm (a version of DuckDB compiled to run
        entirely in your browser). Pick one of the example queries from
        this project's README, or write your own and run it.
      </p>
    ...
    <p>
      Each row you just queried is one real cell. The two plots below place
      every cell on a map, grouped by clustering: cells with similar
      embeddings end up near each other. Each plot uses a different method
      to build that map, so you can compare them.
    </p>
    <div class="cluster-grid">
      ...
      <figure class="panel">
        ...
        <p class="plot-intro">
          This second map takes a different approach. HNSW is an index: a
          structure that quickly finds a cell's nearest neighbors. This
          plot draws the cells it connected.
        </p>
    ...
    <p>
      Try it yourself. Search for a cell by its number, or let the page
      pick one at random. Either way, the plots above will highlight it
    ...
    <div class="panel">
      ...
      <p>
        Every piece above runs on the same real data, end to end. Each
        segmented cell has an embedding, stored in the
        <code>morphem.features.embedding</code> column as a fixed-size
        array: 1152 numbers, always in the same order, from the pretrained
        <a href="https://huggingface.co/CaicedoLab/MorphEm" target="_blank"
           rel="noopener">CaicedoLab/MorphEm</a> model. That fixed length is
        what lets DuckDB compare any two embeddings directly, using its
        built-in <code>array_distance</code> function, with no extra setup.
        This page loads <code>database.jpg</code> into DuckDB-Wasm once,
        then uses that same function to find the nearest neighbors of the
        cell you search for or click. The whole page runs in your browser.
        It does not need a server or Python.
      </p>
      <p>
        The two plots use that same embedding, drawn two different ways.
        Each point in the UMAP projection is one real cell.
        <a href="https://umap-learn.readthedocs.io/" target="_blank"
           rel="noopener">UMAP</a> (McInnes, Healy &amp; Melville, 2018)
        projects each cell's embedding down to two dimensions, using the
        reference <code>umap-learn</code> implementation. It measures
        distance the same way <code>array_distance</code> does above.
      </p>
      <p>
        Each point in the HNSW graph layout is the same cell, placed by a
        different method. First, an
        <a href="https://github.com/nmslib/hnswlib" target="_blank"
           rel="noopener">HNSW</a> index (Malkov &amp; Yashunin, 2016) finds
        each cell's 6 nearest neighbors. This builds a graph: a network of
        cells connected to their neighbors. A force-directed layout (from
        the <a href="https://networkx.org/" target="_blank"
           rel="noopener">networkx</a> library) then arranges the graph so
        that connected cells sit close together. The lines you see are real
        neighbor connections, not a copy of the UMAP layout.
      </p>
    </div>
    <div class="panel finding">
      <h2>Does this match the source paper?</h2>
      <p>
        That clustering connects to something outside this page: the real
        study this data came from. Jason Moffat, a co-author of the study
        that first used this data
        (<a href="https://doi.org/10.1016/j.cell.2006.01.040" target="_blank"
            rel="noopener">Moffat et al. 2006, Cell</a>), provided these
        images. That study screened many genes and used PH3 staining to
        measure "mitotic index": the fraction of cells caught in the act of
        dividing.
      </p>
      <p>
        ...
        tightly together in both plots above. On average,
      ...
      <p>
        MorphEm never trained on this screen. It still recovers the same
        mitotic-versus-resting distinction the original study measured,
        using only image shape and texture. That study's actual gene-by-gene
        results compared many treated wells against controls, and this
        single, untreated field cannot recover those results. But the
        clustering above is a real, checkable link to where this data came
        from.
      </p>
    </div>
    <div class="panel">
  ...
  <script type="module">
    ...
    const renderSqlTable = (table) => {
      ...
      if (fields.length === 0) {
        sqlResultsEl.innerHTML = "<p>Query returned no columns.</p>";
        return;
      }
      const header = fields.map((field) => `<th>${field}</th>`).join("");
      const body = rows
        .map((row) => {
